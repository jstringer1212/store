...
const createUser = async (user, password) => {
    const hashedPassword = await bcrypt.hash(password, 5); // Hash the password with a salt factor of 10
    const SQL = `
...
const createProduct = async (product) => {
    ...
    const response = await client.query(SQL, [uuid.v4, product]);
    return response.rows[0];
};

const createFavorite = async (product_id, user_id) => {
    const SQL = `
    INSERT INTO products (id, product_id, user_id) VALUES ($1, $2, $3) RETURNING *;`;
